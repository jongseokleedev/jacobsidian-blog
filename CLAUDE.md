...
**NEVER edit `src/data/posts/` or `src/data/books/` directly.**
These files are auto-generated by `pnpm run sync` / `pnpm run sync:apply:en` from the Obsidian vault. Any direct edits will be overwritten on the next sync.

- **Korean post edits** → edit the source file in the Obsidian vault (`~/Library/Mobile Documents/iCloud~md~obsidian/Documents/jacobsidian/020.Area/022.Writing/022-1.Public/`), then run `pnpm run sync:apply`
- **English post edits** → edit the source file in the EN vault folders (`Essay-EN/`, `Tech-EN/`), then run `pnpm run sync:apply:en`
